...
describe("Hydra S1 Circuits", () => {
  ...
  // let newInputs: NewPublicInputs & NewPrivateInputs;
  ...
  before(async () => {
    ...
    for(let i = 0; i < 10; i++) {
        // Datatype: account identifier -> address
        const address = (await signers[i].getAddress()).toLowerCase();
        const signature =  await signers[i].signMessage(getOwnershipMsg(address));  
        // Datatype: secret -> bigint
        const secret = BigNumber.from(i);
        const commitment = poseidon([secret]).toHexString(); // for commitment receipt
        // address -> ecdsa signature -> verified -> sign by eddsa
    ...
    // DataType: externalNullifier -> using credential_id
    ...
    /** registryTree and accountTree2 are not used for testing zkid **/
    merkleTreeData2 = {
      [BigNumber.from(accounts[4].identifier).toHexString()]: 1,
      [BigNumber.from(accounts[5].identifier).toHexString()]: 1,
      [BigNumber.from(accounts[6].identifier).toHexString()]: 1,
    ...
    source = accounts[0];
    destination = accounts[4]; // destination should be replaced
    // source is default 1
